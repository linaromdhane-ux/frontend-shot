...
import { Heart, ShoppingCart, User, Moon, Globe, X, Trash2 } from 'lucide-react';
...
const Home = () => {
  ...
  return (
    <div className="relative min-h-screen w-full font-['Montserrat'] bg-[#0f1a18] overflow-x-hidden">
      <style>{`
        ...
        * { font-family: 'Montserrat', sans-serif; }
        ...
        .nav-link-item { color: white; opacity: 0.5; font-weight: 600; font-size: 15px; transition: all 0.3s ease; }
      ...
      {/* --- WISHLIST SIDEBAR --- */}
      <div className={`fixed top-0 right-0 h-full w-[400px] bg-white z-[200] shadow-[-10px_0_30px_rgba(0,0,0,0.2)] transition-transform duration-500 ease-in-out ${isWishlistOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-8 h-full flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-bold text-[#0f1a18]">Wishlist</h2>
            <button onClick={closeSidebars} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
          ...
          <button 
            ...
            className={`w-full py-3.5 rounded-full font-bold mt-6 flex items-center justify-center gap-3 transition-all active:scale-95 border-2 ${
                wishlistItems.length === 0 
                ? 'bg-gray-50 text-gray-300 border-gray-200 cursor-not-allowed' 
                : 'bg-white text-[#333] border-[#238d7b] hover:bg-gray-50'
            }`}
          >
            Clear Wishlist 
            {isClearing && (
              <Trash2 size={20} className="text-[#238d7b] animate-bounce" />
            )}
          </button>
        </div>
      </div>

      {/* --- SHOP SIDEBAR (CHARIOT) --- */}
      <div className={`fixed top-0 right-0 h-full w-[400px] bg-white z-[200] shadow-[-10px_0_30px_rgba(0,0,0,0.2)] transition-transform duration-500 ease-in-out ${isShopOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-8 h-full flex flex-col">
          <div className="flex items-center justify-between mb-8">
      ...
      <div className={(isWishlistOpen || isShopOpen) ? "pointer-events-none select-none" : ""}>
        ...
        >
            <div className="fixed top-0 left-0 z-[100] w-full pt-6 px-10 pointer-events-auto">
            <nav className="mx-auto max-w-7xl nav-fixed-video rounded-full px-10 flex items-center justify-between shadow-2xl">
                <div className="flex-shrink-0">
                <img src="/images/negative-logo-shot-Photoroom 1.png" alt="S.HOT" className="h-9 w-auto" />
                </div>
                <div 
                ...
                <Link to="/register"><button className="btn-signup-vid ml-2">Sign Up</button></Link>
                </div>
            </nav>
            </div>

            <div className="relative z-10 container mx-auto px-12 pt-32 text-white">
              <div className="flex items-center gap-3 mb-6 bg-black/20 w-fit px-4 py-1.5 rounded-full backdrop-blur-md border border-white/10">
                  <div className="w-2 h-2 bg-[#238d7b] rounded-full animate-ping"></div>
                  <span className="text-[11px] font-bold uppercase tracking-[0.2em]">Experience the Power of Spirulina</span>
              </div>

              <h1 className="text-7xl font-black leading-[1.1]">
                  Premium <br /> Spirulina Supplement <br />
                  For <span className="text-[#238d7b] font-kemangi text-9xl ml-4 inline-block">
                  {displayText}<span className="animate-pulse text-white/50 font-sans text-5xl ml-1">|</span>
                  </span>
              </h1>
              
              <p className="text-white/90 mt-10 max-w-lg font-semibold text-lg leading-relaxed">
                  Boost your energy and health with our 100% natural spirulina products.
              </p>

              <div className="flex items-center gap-8 mt-12">
                  <button className="bg-[#1A1A1A] text-white px-10 py-4 rounded-full font-bold flex items-center gap-3 hover:bg-[#238d7b] transition-all shadow-2xl group border border-white/10 pointer-events-auto">
                  Shop now <span className="bg-white text-black rounded-full w-6 h-6 flex items-center justify-center text-sm font-black group-hover:translate-x-1 transition-transform">→</span>
                  </button>
              </div>
            </div>

            {/* --- IMAGE MISE À JOUR ICI --- */}
            <div className="absolute right-[-2%] bottom-0 w-[50%] h-[90%] flex justify-center items-end z-10 pointer-events-none">
            <div className="relative animate-float pointer-events-auto">
                <img 
                    src="/public/images/Group_76.png" 
                    alt="SHOT Product Group" 
                    className="max-h-[700px] w-auto object-contain drop-shadow-[0_35px_35px_rgba(0,0,0,0.5)]" 
                />
            </div>
            </div>
        </header>

        <section className="relative z-10 py-32 px-12 bg-[#0f1a18]">
            <div className="max-w-7xl mx-auto">
            <h2 className="text-white text-5xl font-bold mb-16 font-kemangi">Why Choose Us?</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                <div className="group h-72 bg-[#1a2e2a] rounded-3xl border border-white/5 p-10 text-white hover:border-[#238d7b]/50 transition-all duration-300 hover:-translate-y-2">
      ...
      <div className="h-[400px] bg-[#0f1a18]"></div>
    </div>
  );
};
