  ...
  return (
    ...
    >
      ...
      <div className="absolute top-8 left-8">
        ...
        >
          ...
          <div className={`transition-all duration-500 overflow-hidden ${isBackHomeHovered ? 'max-w-[140px] ml-4 opacity-100' : 'max-w-0 opacity-0 ml-0'}`}>
            <span className="font-bold whitespace-nowrap text-base">{t('forgotPassword.backToHome')}</span>
          </div>
        </Link>
      </div>
      ...
      <div className="w-full max-w-[500px] bg-white rounded-[40px] p-10 shadow-sm flex flex-col items-center border border-gray-100 text-center">
        ...
        <p className="text-gray-500 text-sm mb-10 px-4">{t('forgotPassword.subtitle')}</p>

        <div className="w-full space-y-6 text-left">
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-gray-700 ml-1">{t('forgotPassword.email')}</label>
            <input 
              type="email" 
