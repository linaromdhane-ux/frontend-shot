    ...
    <div className="relative min-h-screen w-full font-['Montserrat'] bg-[#0c1312] overflow-x-hidden">
      <style>{`
        ...
        * { font-family:'Montserrat',sans-serif; }
        ...
        .nav-link-item { color:white; opacity:.8; font-weight:600; font-size:15px; transition:all .3s ease; cursor:pointer; white-space:nowrap; }
        .nav-link-item:hover,.nav-link-active { opacity:1; text-shadow:0 0 8px rgba(255,255,255,.5); }
        .icon-box-vid { width:44px; height:44px; display:flex; align-items:center; justify-content:center; border-radius:14px; transition:all .2s ease; color:white; cursor:pointer; position:relative; flex-shrink:0; }
        .icon-box-vid:hover,.icon-box-active { background-color:white !important; color:#238d7b !important; transform:translateY(-2px); }
        .btn-signup-vid { background-color:white; color:#238d7b; font-weight:700; font-size:14px; height:42px; padding:0 25px; border-radius:50px; border:2px solid white; transition:all .3s ease; cursor:pointer; white-space:nowrap; flex-shrink:0; }
        ...
        .mobile-menu-sidebar {
          ...
          max-width:380px;
          height:100vh;
          background:linear-gradient(135deg, rgba(35,141,123,0.98) 0%, rgba(29,110,96,0.98) 100%);
        ...
        .mobile-menu-sidebar.open { transform:translateX(0); animation:slideInRight 0.4s ease; }
        .mobile-menu-sidebar.closed { transform:translateX(100%); }
      ...
      {/* MOBILE MENU SIDEBAR */}
      <div className={`mobile-menu-sidebar ${isMobileMenuOpen ? 'open' : 'closed'}`}>
        <div className="mobile-menu-header">
          <img src="/images/shot2.png" alt="S.HOT" className="mobile-menu-logo" />
        ...
        <div className="mobile-menu-links">
          {navLinks.map((item) => {
            ...
            const icon = item === 'Home' ? '🏠' : item === 'Products' ? '🛍️' : item === 'About us' ? 'ℹ️' : '📧';
            return (
              <Link 
                key={item} 
                to={linkPath} 
        ...
        <div className="mobile-menu-footer">
          <Link to="/register" onClick={() => setIsMobileMenuOpen(false)}>
            <button className="mobile-menu-signup">Sign Up</button>
          </Link>
        </div>
      </div>
